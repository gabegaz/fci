...
app = DjangoDash('LoanDisbursementClient', add_bootstrap_links=True, 
...
df_level   =  pd.read_excel(data+'loan_disbursement_client.xlsx',
                sheet_name='loan_disbursement_client_level',
                skiprows=1, nrows=40, usecols='A:E')

df_share   =  pd.read_excel(data+'loan_disbursement_client.xlsx',
                sheet_name='loan_disbursement_client_share',
                skiprows=1, nrows=40, usecols='A:E')


df_growth   =  pd.read_excel(data+'loan_disbursement_client.xlsx',
                sheet_name='loan_disbursement_client_growth',
                skiprows=1, nrows=40, usecols='A:E')


df_contrib  =  pd.read_excel(data+'loan_disbursement_client.xlsx',
                sheet_name='loan_disbursement_client_contri',
                skiprows=1, nrows=40, usecols='A:E')
...
def level(client):
    ...
    fig = px.line(dff_level, x= 'year',
                  y = 'level',
                  color='Client',
                  title = "LD #5: Loan Disbursements (Millions of Birr)-Client",
                  labels={
                     "year": "Time in year",
...
def share(client):
    ...
    fig = px.line(dff_share, x= 'year',
                  y = 'share',
                  color='Client',
                  title = "LD #6: Share of Loan Disbursements (Percent)-Client",
                  labels={
                     "year": "Time in year",
                     "share": "Share (percent)",
                     }
...
def growth(client):
    ...
    fig = px.line(dff_growth, x= 'year',
                  y = 'growth',
                  color='Client',
                  title = "LD #7: Growth Rate of Loan Disbursements(Percent)-Client", 
                  labels={
                     "year": "Time in year",
...
def contribution(client):
    ...
    fig = px.line(dff_contrib, x= 'year',
                  y = 'contrib',
                  color='Client',
                  title = "LD #8: Contribution to Growth of Loan Disbursements(Percent)-Client", 
                  labels={
                     "year": "Time in year",

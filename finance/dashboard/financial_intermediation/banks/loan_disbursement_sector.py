...
app = DjangoDash('LoanDisbursementSector', add_bootstrap_links=True, 
...
df_level   =  pd.read_excel(data+'loan_disbursement_sector.xlsx',
                sheet_name='loan_disbursement_sector_level',
                skiprows=1, nrows=39, usecols='A:N')

df_share   =  pd.read_excel(data+'loan_disbursement_sector.xlsx',
                sheet_name='loan_disbursement_sector_share',
                skiprows=1, nrows=39, usecols='A:N')


df_growth   =  pd.read_excel(data+'loan_disbursement_sector.xlsx',
                sheet_name='loan_disbursement_sector_growth',
                skiprows=1, nrows=39, usecols='A:N')


df_contrib  =  pd.read_excel(data+'loan_disbursement_sector.xlsx',
                sheet_name='loan_disbursement_sector_contri',
                skiprows=1, nrows=39, usecols='A:N')
...
def level(sector):
    ...
    fig = px.line(dff_level, x= 'year',
                  y = 'level',
                  color='Sector',
                  title = "LD #9: Loan Disbursements(Millions of Birr)-Sector",
                  labels={
                     "year": "Time in year",
...
def growth(sector):
    dff_share = df_share[df_share['Sector'].isin(sector)]
    fig = px.line(dff_share, x= 'year',
                  y = 'share',
                  color='Sector',
                  title = "LD #10: Share of Loan Disbursements (Percent)-Sector",
                  labels={
                     "year": "Time in year",
                     "share": "Share (percent)",
                     }
...
def share(sector):
    ...
    fig = px.line(dff_growth, x= 'year',
                  y = 'growth',
                  color='Sector',
                  title = "LD #11: Growth Rate of Loan Disbursements(Percent)-Sector", 
                  labels={
                     "year": "Time in year",
...
def contribution(sector):
    ...
    fig = px.line(dff_contrib, x= 'year',
                  y = 'contrib',
                  color='Sector',
                  title = "LD #12: Contribution to Growth of Loan Disbursemtnts(Percent)-Sector", 
                  labels={
                     "year": "Time in year",

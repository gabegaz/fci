...
app = DjangoDash('LoanCollectionBanks', add_bootstrap_links=True, 
...
df_level   =  pd.read_excel(data+'loan_collection_banks.xlsx',
                sheet_name='loan_collection_banks_level',
                skiprows=1, nrows=40, usecols='A:W')

df_share   =  pd.read_excel(data+'loan_collection_banks.xlsx',
                sheet_name='loan_collection_banks_share',
                skiprows=1, nrows=40, usecols='A:W')


df_growth   =  pd.read_excel(data+'loan_collection_banks.xlsx',
                sheet_name='loan_collection_banks_growth',
                skiprows=1, nrows=40, usecols='A:W')


df_contrib  =  pd.read_excel(data+'loan_collection_banks.xlsx',
                sheet_name='loan_collection_banks_contrib',
                skiprows=1, nrows=40, usecols='A:W')
...
def level(lender):
    ...
    fig = px.line(dff_level, x= 'year',
                  y = 'level',
                  color='Lender',
                  title = "LC #1: Collection of Loans (Millions of Birr)-Lender",
                  labels={
                     "year": "Time in year",
...
def growth(lender):
    dff_share = df_share[df_share['Lender'].isin(lender)]
    fig = px.line(dff_share, x= 'year',
                  y = 'share',
                  color='Lender',
                  title = "LC #2: Share in Collection of Loans (Percent)-Lender",
                  labels={
                     "year": "Time in year",
                     "share": "Share (percent)",
                     }
...
def share(lender):
    ...
    fig = px.line(dff_growth, x= 'year',
                  y = 'growth',
                  color='Lender',
                  title = "LC #3: Growth Rate in Collection of Loands (Percent)-Lender", 
                  labels={
                     "year": "Time in year",
...
def contribution(lender):
    ...
    fig = px.line(dff_contrib, x= 'year',
                  y = 'contrib',
                  color='Lender',
                  title = "LC #4: Contribution to Growth in Collection of Loans (Percent) -Lender", 
                  labels={
                     "year": "Time in year",
